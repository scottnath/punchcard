      ...
      if (_.has(attrs, attr.id)) {
        ...
        result[attr.id] = items.map((item, index) => {
          ...
          inputs.forEach((inp) => {
            ...
            // another fix until content-types can be fixed to handle arrays properly
            let inpt = attr.inputs[inp];
            if (Array.isArray(attr.inputs)) {
              inpt = attr.inputs[0][inp];
            }

            // add reference object, if there is one
            const ref = reference(value, inpt, `${attr.id}[${index}].${inp}`, references);
